...
class ReceiptImage {
  constructor(imageSrc) {
    this.originalSrc = imageSrc;
    this.compressedSrc = null;
    this.detectedText = null;
    this.detectedPrice = null;
  }
  ...
  async compress() {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        this.compressedSrc = canvas.toDataURL("image/jpeg", 0.7);
        resolve(this.compressedSrc);
      };
      img.onerror = () => {
        console.error("Error loading image for compression");
        this.compressedSrc = this.originalSrc;
        resolve(this.originalSrc);
      };
  ...
  async performOCR() {
    try {
      ...
      const response = await fetch(this.compressedSrc);
  ...
  detectPrice() {
    ...
    for (const pattern of pricePatterns) {
      const match = this.detectedText?.match(pattern);
      if (match) {
        this.detectedPrice = parseFloat(match[1]);
        break;
      }
    }
  }

  async saveToFirestore(confirmedPrice) {
    const receiptData = {
      imageData: this.compressedSrc,
      price: confirmedPrice,
      timestamp: serverTimestamp(),
      ocrText: this.detectedText,
      imageUrl: `data:image/jpeg;base64,${this.compressedSrc.split(",")[1]}`,
    };

    return await addDoc(collection(db, "receipts"), receiptData);
  }
}
...
const Camera = ({ onClose }) => {
  ...
  const handlePriceConfirmation = async () => {
    ...
    try {
      const docRef = await receiptImage.saveToFirestore(confirmedPrice);
  ...
  const capture = useCallback(async () => {
    ...
    if (webcamRef.current) {
      ...
      const newReceiptImage = new ReceiptImage(imageSrc);
      setReceiptImage(newReceiptImage);
      setIsProcessing(true);
      setError(null);
      ...
      try {
        await newReceiptImage.performOCR();
        if (!newReceiptImage.detectedPrice) {
          setError(
        ...
        setConfirmedPrice(newReceiptImage.detectedPrice);
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="text-sm text-blue-700">
                ...
                <p className="font-semibold">
                  RM {receiptImage?.detectedPrice?.toFixed(2)}
                </p>
              </div>
            </div>
